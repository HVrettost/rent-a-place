package com.rentaplace.auth.filter;

import com.auth0.jwt.exceptions.JWTVerificationException;
import com.rentaplace.auth.config.enumeration.FreeForAllEndpoints;
...
public class AuthorizationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (!httpRequest.getRequestURI().startsWith("/rentaplace/v")
                ...
                || Arrays.stream(FreeForAllEndpoints.values()).anyMatch(endpoint -> endpoint.getEndpoint().equals(httpRequest.getRequestURI()))) {
